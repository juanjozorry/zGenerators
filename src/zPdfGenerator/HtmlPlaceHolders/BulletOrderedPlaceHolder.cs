...
namespace zPdfGenerator.HtmlPlaceHolders
{
    ...
    public class BulletOrderedPlaceHolder<T> : BasePlaceHolder<T>
    {
        /// <summary>
        ...
        public BulletOrderedPlaceHolder(string name, Func<T, IEnumerable<string>> map) : base(name)
        {
            this.Map = map;
        }
        ...
        public override void ProcessNode(HtmlNode htmlNode, T dataItem, CultureInfo cultureInfo, ILogger logger)
        {
            ...
            {
                n.ChildNodes.Clear();
                var ul = n.OwnerDocument.CreateElement("ol");
                n.AppendChild(ul);
                foreach (var item in items)
                {
                    var li = n.OwnerDocument.CreateElement("li");
                    li.AppendChild(n.OwnerDocument.CreateTextNode(item));
                    ul.AppendChild(li);
                }
            }
        }
